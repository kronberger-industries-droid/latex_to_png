...
class EquationProcessor:
    ...
    def read_equation_list(self):
        ...
        with open(self.file_path, 'r') as file:
            ...
            for row in reader:
                if row[0] == '1':  # Filtering valid rows assuming '1' indicates a valid entry
    ...
    def compile_latex_file(self, tex_file):
        """Compiles a LaTeX file into a PDF and logs the compiler output."""
        try:
            ...
            if result.returncode != 0:
                ...
                logging.info(f"Compiled LaTeX file successfully: {os.path.basename(tex_file)}")
                logging.info(f"LaTeX Compiler Output:\n{result.stdout}")
        except subprocess.CalledProcessError as e:
            logging.error(f"LaTeX compilation failed for {os.path.basename(tex_file)}: {e.stderr}")
    ...
    def convert_pdf_to_png(self, pdf_file, png_file):
        ...
        try:
            ...
            subprocess.run(
                ["magick", "convert", "-density", str(self.resolution), "-quality", "100", pdf_file_unix, png_file_unix],
                stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=True
            )
            logging.info(f"Converted PDF to PNG: {os.path.basename(pdf_file)}")
        except subprocess.CalledProcessError as e:
            logging.error(f"PDF to PNG conversion failed for {os.path.basename(pdf_file)}: {e}")
        ...
        for root, dirs, files in os.walk(self.output_path):
            for file in files:
                if any(file.endswith(ext) for ext in file_extensions):
                    ...
                    deleted_files += 1
        logging.info(f"Deleted {deleted_files} intermediate files and build directories.")
    ...
    def process_equations(self):
        ...
        confirmation = input("Do you want to proceed with these equations? (y/n): ").strip().lower()
        if confirmation != 'y':
        ...
        for i, (equation, filename) in enumerate(equations, start=1):
            ...
            base_filename = self.clean_filename(filename if filename else f"equation_{i}")
...
def main():
    parser = argparse.ArgumentParser(description="Process equations from a CSV file into PNG images.")
    ...
    parser.add_argument("--color", default="#464445", help="Equation color in hex format. (standard gray: #2b363a, black: #000000, red: #ff0000, blue: #0000ff, green: #00ff00, etc.)")
    args = parser.parse_args()
    if not args.file_path:
        args.file_path = input("Enter the path to the CSV file: ").strip()
    processor = EquationProcessor(args.file_path, args.output_path, args.resolution, args.color)
